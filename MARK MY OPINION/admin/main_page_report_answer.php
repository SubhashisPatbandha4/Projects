...
$query="select * from report_question_comment order by reportid desc";
$db=new Database();
...
//   print_r($user);
  ...
  $query2="select * from comments where comment_id='$comment_id'";
  $answer= $db->read($query2);
            ...
            <?php 
                    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                      ...
                      if(isset($_POST['ignore']))
                      {
                        ...
                        $query="delete from report_question_comment where reportid ='$reportid'";
                        $result=$db->delete($query);
                        if($result){
                            echo"<script>alert('report deleted from report_question_comment table');</script>";
                            header("Location:main_page_report_answer.php");
                        }else
                        {
...
<form method="post">
            ...
            <div class="admin_action">
                ...
                <input type="hidden" name="cid" value="<?php echo $answer[0]['comment_id'];?>">
